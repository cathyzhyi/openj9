...
J9::X86::TreeEvaluator::stringCaseConversionHelper(TR::Node *node, TR::CodeGenerator *cg, CaseConversionManager &manager)
  ...
  if (!manager.isCompressedString())
      {
      generateRegImmInstruction(CMP4RegImms, node, singleChar, 0, cg);
      generateLabelInstruction(JL4, node, failLabel, cg);
      }
   generateRegImmInstruction(CMP4RegImms, node, singleChar, 0x80, cg);
   generateLabelInstruction(JAE4, node, failLabel, cg);

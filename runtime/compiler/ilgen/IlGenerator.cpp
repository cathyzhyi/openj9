 ...
 *
 * In the world without value type, a ResolveCHK is emitted before the checkcast,
 * and both the ResolveCHK and checkcast are skipped when the object tested is null.
 *
 * In the world with value type, NullPointerException needs to be thrown when the object
 * tested is null and the checkcast class is value type. A ResolveCHK is emitted before
 * the checkcast and applies to all cases. Checkcast applies only when the object tested
 * is non null. A NullCHK applies when object tested is null and checkcast class is value
 * type.
 *
 ...
 *
   ...
   @endverbatim
 *
 * in the world without value type, into this:
 *
   @verbatim
         +-<block_$orig>------------------+
         |  ... $pre                      |
   ...
   @endverbatim
 *
 * in the world with value type, into this:
 *
  @verbatim
        +->block_$orig<------------------------------+
        |  ... $pre                                  |
        ...
        |  ... astore $tmp_obj                       |
        |  n2n   =>$obj                              |
        |  ... ResolveCHK                            |     +--block_$null_case-------------------+
        |  n3n   loadaddr $klass [unresolved Static] |     |  ...ificmpeq --> block_$merge       |
        |  ... astore $tmp_class                     |     |        iand                         |
        |  n3n   =>loadaddr $klass                   +---->+           iloadi j9class.classFlags +--+
        |  ... (more fixupCommonning)                |     |           aload $temp_class         |  |
        |  ... ifacmpeq --> block_$null_case         |     |        iconst J9ClassIsValueType    |  |
        |  n2n   =>$obj                              |     +------------+------------------------+  |
        |  ...   aconst NULL                         |                  |                           |
        +--------------+-----------------------------+                  |                           |
                       |                                    fallthrough |                           |
           fallthrough |                                                |                           |
                       |                                                v                           |
                       v                                   +--block_$null_case_value_type--------+  |
        +->block_$nonnull<--------+                        |  ...NULLCHK                         |  |
        |  n1n checkcast          |                        |        passThrough                  |  |
        |  ...   aload $tmp_obj   |                        |           aload $tmp_obj            |  |
        |  n3n   =^loadaddr       |                        |  ...goto block_$merge               |  |
        +-------------------------+                        +------------+------------------------+  |
                       |                                                |                           |
           fallthrough |        +---------------------------------------+                           |
                       |        |                                                                   |
